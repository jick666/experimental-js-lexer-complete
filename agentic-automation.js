...
function tryRun(cmd) {
  try { run(cmd); return true; } catch (err) { return false; }
}
...
async function openPr() {
  ...
  await octokit.rest.pulls.create({ owner, repo, head: branch, base: 'main', title: `[agent] ${title}`, body });
}
...
(async () => {
  ...
  run(`git checkout -B ${branch}`);
  rebaseMain();
  // edits would be applied here by the calling process
  runChecks();
  rebaseMain();
  runChecks();
